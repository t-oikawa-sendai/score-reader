...
| Related Documents（関連文書） | README.md / 02_REQUIREMENTS_DEFINITION.md / 05_ARCHITECTURE_DESIGN.md / NOTICE / docs/reviews/2026-09-06_SCORE_READER_DESIGN_REVIEW.md |
...
| 未解決 | 個別利用条件と世界向け GitHub 公開の可否は TBD-005 |
...
準拠法域および公開可否の最終判断は未解決である（TBD-005）。本節は法的助言ではない。
...
| テスト素材 | リポジトリに含める場合は §5.4 の分離確認を要する。公開可否は TBD-005 |
...
| リスク | 対策方針 |
|---|---|
| 著作権保護楽譜のリポジトリ混入 | テスト素材は §5.4 の分離確認を経る。公開可否は TBD-005 |
...
| TBD-005 | テスト素材および世界向け GitHub 公開の権利判断。確認対象は次を分離する: 楽曲・作曲物の権利状態 / MusicXML エンコーディングの権利・利用条件 / OMR・変換・編集による派生成果物の扱い / 公開 GitHub からの再配布が許可される範囲。準拠法域および公開可否の最終判断責任者は未確定。 | Takashi Oikawa | 未定 | Open |

由来・個別利用条件・再配布可否の追跡は `05_ARCHITECTURE_DESIGN.md` TBD-005 も参照する。
...
3. **テスト素材の権利確認**: `prototype/tests/` 追加前に、楽曲の権利、MusicXML エンコーディングの権利、加工・再配布条件を分けて確認すること（§5.4、TBD-005 参照）。パブリックドメイン楽譜であることだけで公開可能としない。
