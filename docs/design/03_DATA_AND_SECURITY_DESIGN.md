...
| 未解決 | MusicXML エンコーディングの個別利用条件、世界向け再配布可否の法的確定、準拠法域（TBD-005） |
...
| TBD-005 | テスト素材の未解決権利事項。MusicXML エンコーディングの個別利用条件、世界向け再配布可否の法的確定、準拠法域。最終判断責任者（Takashi Oikawa）、暫定措置（権利確認中である旨を明示して公開継続）は確定済みであり、本 TBD の対象外。公開判断の固定期限は設けない。 | Takashi Oikawa | 未定 | Open |

権利・法務未解決事項の一次追跡先は本文書 TBD-005 である。技術的由来の追跡は `05_ARCHITECTURE_DESIGN.md` TBD-005 を参照する。
...
3. **テスト素材の権利確認**: `prototype/tests/` 追加前に、楽曲の権利、MusicXML エンコーディングの権利、加工・再配布条件を分けて確認すること（§5.4、TBD-005 参照）。パブリックドメイン楽譜であることだけで公開可能としない。現行公開中素材は権利確認中の暫定公開継続である。最終判断責任者は Takashi Oikawa。公開判断の固定期限は設けない。未解決はエンコーディング個別利用条件、再配布可否の法的確定、準拠法域に限定する。技術的由来は `05_ARCHITECTURE_DESIGN.md` TBD-005。
